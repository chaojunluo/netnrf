...
namespace Netnr.Func
{
    public class Common
    {
                        ...
                        case "EndsWith":
                            {
                                ...
                                query = query.Where(Core.LamdaTo.Contains<T>(field, iwhere));
                            }
                            break;
                        case "BetweenAnd":
                            if (value.Count() == 2)
        ...
        /// </summary>
        /// <returns></returns>
        public static LoginUserVM GetLoginUserInfo()
        {
            var loginUser = new LoginUserVM();
            try
            {
                var hc = System.Web.HttpContext.Current.Request.Cookies["__AUTH"];

                if (hc != null && !string.IsNullOrWhiteSpace(hc.Value))
                {
                    var jo = Core.CalcTo.DeDES(hc.Value, "netnr").ToJObject();
                    var pis = loginUser.GetType().GetProperties();
                    foreach (var pi in pis)
                    {
                        pi.SetValue(loginUser, jo[pi.Name].ToString());
                    }
                    return loginUser;
                }
            }
        ...
        public static SysRole LoginUserRoleInfo()
        {
            var lui = GetLoginUserInfo();
