﻿/// <reference path="../lib/jquery/jquery-1.12.4.js" />
...
(function (window, undefined) {
    ...
    z.Grid.fn = z.Grid.prototype = {
        ...
        bind: function () {
            ...
            //复选框
            var hasck = false;
            $(that.frozenColumns[0]).each(function () {
                if ("checkbox" in this) {
                    hasck = true;
                    return false;
                }
            })
            if (that.checkbox && !hasck) {
                that.frozenColumns[0].splice(0, 0, { field: "ck", checkbox: true });
            }
            if (!that.checkbox && hasck) {
                $(that.frozenColumns[0]).each(function () {
                    if ("checkbox" in this) {
            ...
            if (typeof that._onBeforeLoad == "function") {
                that.onBeforeLoad = that._onBeforeLoad;
                delete that._onBeforeLoad;
            }
            ...
            if (that.pagination) {
                ...
                //自定义页量处理
                var arrps = [10, 20, 30, 50], hasps = false, ps = that._pageSize || that.pageSize;
                for (var u = 0; u < arrps.length; u++) {
                    if (arrps[u] == ps) {
                        hasps = true;
                        break;
                    }
                ...
                if (!hasps) {
                    if (!that._pageSize) {
                        that._pageSize = that.pageSize;
                    }
                    arrps.push(that._pageSize);
                    arrps = arrps.sort(function (x, y) {
                        return x > y;
                    });
                }
    ...
    z.FormAttrBind = function (target) {
        ...
        switch (dtype) {
            ...
            case 'time':
                {
                    var format1 = "yyyy-MM-dd", format2 = "HH:mm:ss", obj = {
                    ...
                    target.parent().datetimepicker(obj);
                    target.attr('data-state', 'bind');
                }
                break;
            case 'modal':
    ...
    z.FormRequired = function (color, FormId, dialog) {
        ...
        if (!result && !dialog) {
            ...
            mo.modal.find('h4.modal-title').html('<b class="red">蓝色标题项为必填</b>');
        }
        return result;
    }
    ...
    z.FormEdit = function (rowData) {
        for (var i in rowData) {
            var t = $('#' + i), rdi = rowData[i], dtype = t.attr('data-type') || t.attr('type');
            switch (dtype) {
                case 'combobox':
                case 'combotree':
    ...
    z.FormTitle = function (ops) {
        ...
        required && htm.push(' <em>蓝色标题项为必填</em>');
    ...
    z.SqlQuery.fn = z.SqlQuery.prototype = {
        ...
        private: {
            ...
            //示例格式 
            //(id1='1' and (id2 IN('1','2') or id2 LIKE '%5%') and id3 BETWEEN '5' AND '10')
            wheres: {
                andor: "and",
                items: [
                    {
...
if ($.fn.datagrid) {
    ...
    //datetime 日期时间
    ge.datetime = bo();
    ge.datetime.init = function (container, options) {
        var input = $('<input class="datagrid-editable-input" />').appendTo(container);
    ...
    //date 日期
    ge.date = bo();
    ge.date.init = function (container, options) {
        var input = $('<input class="datagrid-editable-input" />').appendTo(container);
    ...
    //time 时间
    ge.time = bo();
    ge.time.init = function (container, options) {
        var input = $('<input class="datagrid-editable-input" />').appendTo(container);
        input.datetimepicker({ format: "hh:mm:ss", pickDate: false });
        input[0].focus();
        return input;
    };
    ...
    //password 密码
    ge.password = bo();
    ge.password.init = function (container, options) {
