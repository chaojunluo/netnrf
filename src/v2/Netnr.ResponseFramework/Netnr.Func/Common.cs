    ...
    public class Common
    {
        ...
        public static Dictionary<string, string> DicTable
        {
            get
            {
                if (_DicTable == null)
                {
                    var ass = Assembly.Load(DataBase.ProjectAttr.Domain).GetTypes();
        ...
        public static void QueryJoin<T>(IQueryable<T> query, QueryDataVM.GetParams param, RepositoryUse ru, ref QueryDataVM.OutputResult or)
        {
            ...
            //排序
            param.sortOrderJoin = DataBase.OrderByJoin(param.sort, param.order, false);
            query = DynamicQueryableExtensions.OrderBy(query, param.sortOrderJoin);
            //数据
