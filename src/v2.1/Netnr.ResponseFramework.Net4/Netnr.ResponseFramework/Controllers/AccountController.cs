...
namespace Netnr.ResponseFramework.Controllers
{
    public class AccountController : Controller
    {
        ...
        public FileResult Captcha()
        {
            ...
            var hc = new HttpCookie("captcha", Core.CalcTo.MD5(num.ToLower()));
        ...
        [Description("登录验证")]
        [AllowAnonymous]
        public string LoginValidation(SysUser mo, string captcha, int remember, bool isString = true)
        {
            return LoginValidation(mo, captcha, remember).ToJson();
        }
        ...
        private AccountValidationVM LoginValidation(SysUser mo, string captcha, int remember)
        {
            ...
            else
            {
                ...
                using (var db = new ContextBase())
                {
                    ...
                    outMo = db.SysUser.Where(x => x.UserName == mo.UserName && x.UserPwd == mo.UserPwd).FirstOrDefault();
                }
            }
            ...
            try
            {
                ...
                var lu = new LoginUserVM()
                {
                    RoleId = outMo.RoleId,
                    UserId = outMo.Id,
                    UserName = outMo.UserName,
                    Nickname = outMo.Nickname
                };

                HttpCookie hc = new HttpCookie("__AUTH");
                hc.Value = Core.CalcTo.EnDES(lu.ToJson(), "netnr");
                if (isRemember)
                {
                ...
                //登录票据
                FormsAuthentication.SetAuthCookie(outMo.Id, isRemember);
